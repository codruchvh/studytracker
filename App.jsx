...
/* ============================================================================
   ...
   serif reserved for the handful of moments that should carry personality,
   and a monospace kept strictly for actual data (numbers, dates, counts) —
   not decoration. No glow, no idle animation.
============================================================================ */
const STYLES = `
...
/* segmented control — range toggles, timer mode, duration presets */
...
/* status pulse — only animates while genuinely active */
...
/* two-column layout for Notes (avoids Tailwind arbitrary-value classes) */
...
/* Deliberately empty — this is a genuine blank slate. Subjects, sessions,
   notes, sources, tasks and flashcards only ever come from what you add. */
function createEmptyStudyState() {
  return { subjects: [], notes: [], sources: [], tasks: [], flashcards: [], sessions: [] };
}

/* ============================================================================
   HELPERS — dates, formatting, calendar/analytics math, local persistence.
   No network calls, no external services: everything here is a pure
   function of whatever you've actually entered into this app.
============================================================================ */
let idCounter = 1000;
...
function buildTrendBuckets(sessions, range) {
  ...
  // all: monthly buckets across the full session history
...
/* short, synthesized chime — no external asset, no network call */
function playChime() {
  try {
  ...
  } catch (e) { /* ignore — audio is a nicety, never a requirement */ }
}

/* ---------- persistence: plain browser localStorage, nothing Claude-specific ---------- */
...
/* ============================================================================
   STUDY ANALYTICS — time-by-subject donut, consistency heatmap, per-subject
   goal table, session log. Every number is derived from what you've logged;
   nothing here is fabricated or pre-filled.
============================================================================ */
...
function StudyOverview({ study, timer, onStartPause, onResetTimer, onSelectTimerSubject, onTimerModeChange, onTimerTargetChange, onTimerTaskChange, onStartBreak, postSessionPrompt, onDismissPostSession, onRateSession, onCompleteLinkedTask, studyActions }) {
  ...
  return (
    <div className="py-5 space-y-5">
      ...
      {/* interactive timer: stopwatch, focus (with countdown + task link), and break cycles */}
      ...
      {/* trend + time by subject, toggled weekly / monthly / all time */}
      ...
      {/* per-subject table with weekly goals */}
      <div>
...
/* ============================================================================
   NOTES, RESEARCH, PLANNER, FLASHCARDS — plain CRUD, no AI dependency
============================================================================ */
...
export default function App() {
  ...
  const [postSessionPrompt, setPostSessionPrompt] = useState(null); // { sessionId, subjectId, taskId, mode }
  ...
  /* ---------- focus/break completion: auto-log, chime, hand off to reflection ---------- */
  useEffect(() => {
    if (!timer.running) return;
